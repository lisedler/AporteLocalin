...
//<span>Usuario</span>	
	...
	if ( $fail == 'name' ) { $reg_message = __('That username already exists','whatif'); $reg_out .="<span>Usuario</span><br><input class='login-caja error' type='text' name='nombre' value='" . $reg_message . "' onblur=\"if(this.value == '') {this.value = '" . $reg_message . "';}\" onfocus=\"if(this.value == '" . $reg_message . "') {this.value = '';}\" />"; }
	else { $reg_message = __('Username','whatif'); $reg_out .="<span>Usuario</span><br><input class='login-caja ".$color."' type='text' name='nombre' value='" .$reg_message. "' onblur=\"if(this.value == '') {this.value = '" .$reg_message. "';}\" onfocus=\"if(this.value == '" .$reg_message. "') {this.value = '';}\" />"; }
	$reg_out .= "<!-- <span>" . __('No spaces','whatif') . "</span>-->";

	if ( $fail == 'mail' ) { $reg_message = __('This email is already in use by other user','whatif'); $reg_out .="<br><span>Mail</span><br><input class='login-caja error' type='text' name='mail' value='" .$reg_message. "' onblur=\"if(this.value == '') {this.value = '" .$reg_message. "';}\" onfocus=\"if(this.value == '" .$reg_message. "') {this.value = '';}\" />"; }
	else { $reg_message = __('e-mail','whatif'); $reg_out .= "<br><span>Mail</span><br><input class='login-caja ".$color."' type='text' name='mail' value='" .$reg_message. "' onblur=\"if(this.value == '') {this.value = '" .$reg_message. "';}\" onfocus=\"if(this.value == '" .$reg_message. "') {this.value = '';}\" />"; }

	$reg_out .= "
		<br><span>Contraseña</span>		
		<br><input class='login-caja ".$color."' type='password' name='pass' value='' />
		<!-- <span>" . __('Password','whatif') . "</span> -->
		<br><input class='login-caja ".$color."' type='password' name='pass2' value='' />
	";
	...
	$reg_out .=  "
		<br><input type='hidden' name='ref' value='".$perma."' />
		<br><input class='login-boton ".$color."' type='submit' value='" . __('Sign up','whatif') . "' name='registro' />
		<br><span>  </span>
		<br>
	</form>
